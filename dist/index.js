...
const which_1 = __importDefault(__webpack_require__(207));
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const runtimeConfigFilename = `${path_1.default.basename(startupDllName, '.dll')}.runtimeconfig.json`;
            const depsJsonFilename = `${path_1.default.basename(startupDllName, '.dll')}.deps.json`;
            const migrationDllPath = getPathToFileAndVerifyExists(pathToDirectory, migrationsDllName);
            const startupDllPath = getPathToFileAndVerifyExists(pathToDirectory, startupDllName);
            const runtimeConfigPath = getPathToFileAndVerifyExists(pathToDirectory, runtimeConfigFilename);
            const depsJsonPath = getPathToFileAndVerifyExists(pathToDirectory, depsJsonFilename);
...
function getPathToEfDll() {
    ...
    const pathToEfDllOption1 = path_1.default.join(__dirname, 'ef.dll');
    const pathToEfDllOption2 = path_1.default.join(__dirname, '..', 'dist', 'ef.dll');
    ...
    if (option1Exists === false && option2Exists === false) {
        core.setFailed(`Could not locate ef.dll at expected paths: 1) ${pathToEfDllOption1} 2) ${pathToEfDllOption2}`);
        return null;
    }
...
function getPathToFileAndVerifyExists(dirPath, filename) {
    if (!dirPath || dirPath === null || dirPath === '') {
        ...
        const pathToFile = path_1.default.join(dirPath, filename);
